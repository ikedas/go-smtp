...
func decodeUTF8AddrXtext(val string) (string, error) {
	...
	decoded := eUOrDCharRe.ReplaceAllStringFunc(val, func(match string) string {
		if len(match) == 1 {
			replaceErr = errors.New("disallowed character")
			return ""
		}
		...
		switch {
		case l == 2 && (0x01 <= char && char <= 0x09 ||
			0x11 <= char && char <= 0x19 || char == 0x10 ||
			char == 0x20 ||
			// all xtext-specials
			char == 0x2B || char == 0x3D || char == 0x7F ||
			// 2-digit forms
			char == 0x5C || 0x80 <= char && char <= 0xFF):
		// 3-digit forms
		case l == 3 && 0x100 <= char && char <= 0xFFF:
		// 4-digit forms excluding surrogate
		case l == 4 && (0x1000 <= char && char <= 0xD7FF ||
			0xE000 <= char && char <= 0xFFFF):
		// 5-digit forms
		case l == 5 && 0x1_0000 <= char && char <= 0xF_FFFF:
		// 6-digit forms
		case l == 6 && 0x10_0000 <= char && char <= 0x10_FFFF:
		default:
			replaceErr = errors.New("illegal hexpoint")
			return ""
		}
...
func decodeTypedAddress(val string) (OrcptTypeType, string, error) {
	...
	return OrcptTypeType(aType), aAddr, nil
}
...
func (c *Conn) handleRcpt(arg string) {
	...
	for key, value := range args {
		...
		case "NOTIFY":
			...
			value = strings.ToUpper(value)
			if value == "NEVER" {
				opts.Notify = []NotifyType{NotifyType(value)}
			} else {
				opts.Notify = []NotifyType{}
				for _, val := range strings.Split(value, ",") {
					switch val {
					case "DELAY", "FAILURE", "SUCCESS":
					default:
						c.writeResponse(501, EnhancedCode{5, 5, 4}, "Malformed NOTIFY parameter value")
						return
					}
					opts.Notify = append(opts.Notify, NotifyType(val))
				}
			}
		case "ORCPT":
			if !c.server.EnableDSN {
			...
			opts.OrcptType = aType
			opts.OrcptAddr = aAddr
		default:
